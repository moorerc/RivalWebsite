...
var pointACenter = {
    x: canvasCenter.x,
    y: canvasCenter.y - ((Math.sqrt(triangleSize*triangleSize - (triangleSize/2)*(triangleSize/2)))/2),
};
...
function drawBrainCircle() {
    ...
    drawTriangle();
    context.lineWidth = 1;
    document.getElementById("triangleVal").innerHTML=-1*(triangleSize-100)+100;

    var brainCircle =new Path2D();
    context.fillStyle = "rgba(0, 61, 167, 0.3)";
    context.strokeStyle = "rgba(0, 61, 167, 1.0)";
    brainCircle.arc(pointBCenter.x,pointBCenter.y,brainVal,0,Math.PI*2,true);
    ...
    effortCircle.arc(pointACenter.x,pointACenter.y,effortVal,0,Math.PI*2,true);
    ...
    skillCircle.arc(pointCCenter.x,pointCCenter.y,skillVal,0,Math.PI*2,true);
